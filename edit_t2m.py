"""Run motion editing inference with a text prompt and a source motion NPZ file."""
import os
...
if __name__ == '__main__':
    ...
    motion_data = np.load(opt.source_motion, allow_pickle=True)
    ...
    for r in range(opt.repeat_times):
        ...
        for k, (caption, joint_data, source_data)  in enumerate(zip(captions, data, source_data)):
            ...
            source_joint = recover_from_ric(torch.from_numpy(source_data).float(), 22).numpy()
